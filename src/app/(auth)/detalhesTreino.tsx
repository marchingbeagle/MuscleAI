      ...
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            const updatedTreinos = treinos.filter((t) => t.id_treino !== idTreino);
  ...
  return (
    <View className="flex-1 bg-white">
      <ScrollView>
        <View className="p-6">
          <View className="mb-8">
            <Text className="text-2xl font-bold text-[#2f855a]">Treinos de {nomeAluno}</Text>
          ...
          <View className="space-y-4">
            {treinos.length === 0 ? (
              <View className="items-center justify-center p-6 bg-gray-50 rounded-xl">
                ...
                <Text className="mt-2 text-gray-500">Nenhum treino encontrado</Text>
              </View>
            ) : (
              treinos.map((treino, index) => (
                <View key={treino.id_treino} className="mb-4 overflow-hidden bg-gray-50 rounded-xl">
                  <View className="flex-row items-center justify-between p-3 bg-gray-100 border-b border-gray-200">
                    ...
                    <View className="flex-row space-x-2">
                      <TouchableOpacity
                        onPress={() => router.push(`/editarTreino?idTreino=${treino.id_treino}`)}
