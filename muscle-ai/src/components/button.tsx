...
export const ButtonText = ({ style, children }: TextProps) => {
  ...
  return (
    <Text style={[tailwind`font-bold`, variants[variant as ButtonVariant], style]}>
      {children}
    </Text>
  );
};
...
export const ButtonIcon = ({ name, type, color, size }: IconProps) => {
  ...
  const defaultColor =
    colorScheme === "dark" ? variants[variant as keyof typeof variants].dark : variants[variant as keyof typeof variants].light;
